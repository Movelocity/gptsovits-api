...
class Text2SemanticDecoder(nn.Module):
    ...
    def infer_panel(
        ...
        x_attn_mask_pad = F.pad(
            x_attn_mask,
            (0, y_len),  ### xx的纯 0 扩展到 xx 纯 0+xy 纯 1，(x,x+y)
            value=True,
        )
        y_attn_mask = F.pad(  ### yy 的右上 1 扩展到左边 xy 的 0,(y,x+y)
            torch.triu(torch.ones(y_len, y_len, dtype=torch.bool), diagonal=1),
            (x_len, 0),
            value=False,
        )
        ...
        for idx in tqdm(range(1500)):
            ...
            logits = self.ar_predict_layer(
                xy_dec[:, -1]
            )  ## 如果用了cache的默认就是只有一帧，取最后一帧一样的
            # samples = topk_sampling(logits, top_k=top_k, top_p=1.0, temperature=temperature)
            if idx == 0:  ### 第一次跑不能 EOS 否则没有了
                logits = logits[:, :-1]  ### 刨除 1024 终止符号的概率

            # 之前的 y 和本次生成的 semantic_ids 拼接成新的 y
            samples = sample(
                logits[0], y, top_k=top_k, top_p=top_p, repetition_penalty=1.35, temperature=temperature
            )[0].unsqueeze(0)  # print(samples.shape)  # [1,1]  # 第一个 1 是 bs            
            y = torch.concat([y, samples], dim=1) 
            ...
            if stop:
                ...
                if y.shape[1] == 0:
                    y = torch.concat([y, torch.zeros_like(samples)], dim=1)
                    print("bad zero prediction")
                print(f"\nTTS Decoding EOS [prefix: {prefix_len} -> total: {y.shape[1]}]")
                break
